...
namespace visualization
{
    ...
    static ImGui_ImplVulkanH_Window *WD = NULL;
    static std::vector<std::function<void()>> viz_jobs;

    void add_viz_job(std::function<void()> job)
    {
        viz_jobs.push_back(job);
    }
    ...
    void render()
    {
        ...
        if (!is_minimized)
        {
            WD->ClearValue.color.float32[0] = CLEAR.x * CLEAR.w;
            WD->ClearValue.color.float32[1] = CLEAR.y * CLEAR.w;
            WD->ClearValue.color.float32[2] = CLEAR.z * CLEAR.w;
            WD->ClearValue.color.float32[3] = CLEAR.w;
