...
int main(const int argc, const char* argv[])
{
    ...
    try {
        if (is3D) {
            ...
            vector<da_slam::types::Timestep3D> timesteps = convert_into_timesteps(odomFactors3d, measFactors3d);
            ...
            pose_prior_noise = pose_prior_noise.array().sqrt().matrix();  // Calc sigmas from variances
            vector<da_slam::types::Timestep2D> timesteps = convert_into_timesteps(odomFactors2d, measFactors2d);
