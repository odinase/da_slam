...
std::vector<int> Hungarian::solve(const Eigen::Ref<const Eigen::MatrixXd>& cost_matrix) const
{
    const auto n_rows = cost_matrix.rows();
    const auto n_cols = cost_matrix.cols();

    std::vector<int> assignment;
    assignment.resize(n_rows);
    double cost = 0;

    // call solving function
    assignmentoptimal(assignment.data(), &cost, cost_matrix.data(), n_rows, n_cols);
...
constexpr auto DBL_EPSILON = std::numeric_limits<double>::epsilon();
...
void assignmentoptimal(int* assignment, double* cost, const double* distMatrixIn, int nOfRows, int nOfColumns)
{
    ...
    *cost = 0;
    ...
    for (row = 0; row < nOfElements; row++) {
        ...
        if (value < 0) spdlog::error("All matrix elements have to be non-negative.");
