...
Eigen::MatrixXd ellipse2d(const Eigen::Vector2d& mu, const Eigen::Matrix2d& P, const double s = 1.0, const int n = 200)
{
    ...
    points << thetas.array().cos(), thetas.array().sin();
    Eigen::LLT<Eigen::Matrix2d> chol(P);
    Eigen::MatrixXd ell = (s * chol.matrixL().toDenseMatrix() * points).colwise() + mu;
    return ell;
}

std::vector<Eigen::MatrixXd> ellipse3d(const Eigen::Vector3d& mu, const Eigen::Matrix3d& P, const double s = 1.0,
                                       const int num_level_curves = 5, const int n = 200)
{
    ...
    for (int i = 0; i < num_level_curves; i++) {
        ...
        double phi = acos(z);
        circ.topRows(2) << thetas.array().cos() * sin(phi), thetas.array().sin() * sin(phi);
        circ.row(2).array() = z;
...
void draw_factor_graph(const gtsam::NonlinearFactorGraph& graph, const gtsam::Values& estimates, int latest_time_step)
{
    ...
    for (const auto& factor_ : graph) {
        meas2d = std::dynamic_pointer_cast<gtsam::PoseToPointFactor<gtsam::Pose2, gtsam::Point2>>(factor_);
        odom2d = std::dynamic_pointer_cast<gtsam::BetweenFactor<gtsam::Pose2>>(factor_);
        meas3d = std::dynamic_pointer_cast<gtsam::PoseToPointFactor<gtsam::Pose3, gtsam::Point3>>(factor_);
        odom3d = std::dynamic_pointer_cast<gtsam::BetweenFactor<gtsam::Pose3>>(factor_);
    ...
    for (const gtsam::Values::ConstKeyValuePair p : estimates) {
        ...
        if (gtsam::symbolChr(p.key) == 'x' || gtsam::symbolChr(p.key) == '\0') {
            ...
            legend = "Poses";
            ImPlot::SetNextMarkerStyle(ImPlotMarker_Circle, 5.0,
                                       ImVec4(19.0 / 255.0, 160.0 / 255.0, 17.0 / 255.0, 1.0));
        }
...
void draw_covar_ell(const Eigen::Vector2d& l, const Eigen::Matrix2d& S, const double s, const char* covariance_label,
                    const int n)
{
                     ...
                     const std::map<gtsam::Key, bool>& lmk_cov_to_draw)
{
    ...
    std::string table_title = "Mahalanobis threshold = " + std::to_string(sigmas * sigmas) +
                              ", sigma = " + std::to_string(sigmas) +
                              ", probability (chi2 test) = " + std::to_string(ic_prob);
    ...
    for (const auto& association : hypothesis.associations()) {
        ...
        if (association->associated()) {
            ...
            if (lmk_cov_to_draw.at(lmk_key)) {
                // S = Gz * S * Gz.transpose() + Gx * Pxx * Gx.transpose(); // Express S in world frame. Correct to do
                // this way??
        ...
        if (association->associated()) {
            ...
            if (lmk_cov_to_draw.at(lmk_key)) {
                ...
                for (const Eigen::MatrixXd& ell_level_curve : ell_level_curves) {
                    ...
                    int stride = 2 * sizeof(double);
                    ImPlot::PlotLine("Covariance ellipse", &ell_level_curve(0, 0), &ell_level_curve(1, 0), count, 0,
                                     stride);
                }
            }
            // Draw line between measurement and lmk, and cross for measurement
