...
class SLAM
{
   private:
    ...
    void incrementLatestPoseKey()
    {
        latest_pose_key_++;
    }
    void incrementLatestLandmarkKey()
    {
   ...
   public:
    ...
    inline const gtsam::Values& currentEstimates() const
    {
        return estimates_;
    }
    ...
    inline const gtsam::NonlinearFactorGraph& getGraph() const
    {
        return graph_;
    }
    inline double error() const
    {
        return getGraph().error(currentEstimates());
    }
    inline const da::hypothesis::Hypothesis& latestHypothesis() const
    {
        return latest_hypothesis_;
    }
    inline gtsam::Key latestPoseKey() const
    {
        return X(latest_pose_key_);
    }
    inline POSE latestPose() const
    {
        return estimates_.at<POSE>(latestPoseKey());
    }

    inline const gtsam::NonlinearFactorGraph& hypothesisGraph() const
    {
        return hypothesis_graph_;
    }
    inline const gtsam::Values& hypothesisEstimates() const
    {
        return hypothesis_values_;
    }
};
