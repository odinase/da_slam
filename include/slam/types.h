...
struct IndeterminantLinearSystemExceptionWithISAM : public gtsam::IndeterminantLinearSystemException
{
    ...
    std::string when;
    IndeterminantLinearSystemExceptionWithISAM(const gtsam::IndeterminantLinearSystemException& err,
                                               std::unique_ptr<gtsam::ISAM2> isam_, const char* when_) noexcept
    : gtsam::IndeterminantLinearSystemException(err.nearbyVariable()), isam(std::move(isam_)), when(when_)
    {
    }
};
...
struct IndeterminantLinearSystemExceptionWithGraphValues : public gtsam::IndeterminantLinearSystemException
{
    ...
    : gtsam::IndeterminantLinearSystemException(err.nearbyVariable()), graph(graph_), values(values_), when(when_)
    {
    }
};
