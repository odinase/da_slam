...
#define LINESIZE 81920 // dataset.cpp. ignore lines after this
      ...
      const std::string &g2oFile, const bool is3D, const std::string &kernelType)
  {
    ...
    if (!is)
      throw std::invalid_argument("cannot find file " + g2oFile);
    std::string tag;
...
void writeG2o_with_landmarks(const gtsam::NonlinearFactorGraph &graph, const gtsam::Values &estimates, const std::string &filename)
{
  std::ofstream g2o_file(filename, std::ios_base::app);

  boost::shared_ptr<gtsam::PoseToPointFactor<gtsam::Pose2, gtsam::Point2>> measFactor2d;
  boost::shared_ptr<gtsam::PoseToPointFactor<gtsam::Pose3, gtsam::Point3>> measFactor3d;
  for (const auto &factor : graph)
  {
    measFactor2d = boost::dynamic_pointer_cast<gtsam::PoseToPointFactor<gtsam::Pose2, gtsam::Point2>>(factor);
    measFactor3d = boost::dynamic_pointer_cast<gtsam::PoseToPointFactor<gtsam::Pose3, gtsam::Point3>>(factor);
    if (measFactor2d)
    {
    ...
    if (measFactor3d)
    {
      g2o_file << "EDGE_SE3_XYZ " << gtsam::symbolIndex(measFactor3d->key1()) << " " << gtsam::symbolIndex(measFactor3d->key2()) << " " << measFactor3d->measured().transpose() << " ";
      gtsam::Vector sigmas = measFactor3d->noiseModel()->sigmas();
      size_t dim = sigmas.size();
      for (int i = 0; i < dim; i++) {
        for (int j = i; j < dim; j++) {
          g2o_file << (i == j ? 1.0 / (sigmas(i)*sigmas(i)) : 0.0); // we store the information matrix, so store 1/\sigma^2
          g2o_file << (i == dim - 1 ? "\n" : " ");
        }
      }
    }
