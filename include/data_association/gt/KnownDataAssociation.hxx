...
: meas_lmk_assos_(meas_lmk_assos), curr_landmark_count_(0)
{
}

template <class POSE, class POINT>
Hypothesis KnownDataAssociation<POSE, POINT>::associate(const gtsam::Values& estimates,
    ...
    for (const auto& asso : h.associations()) {
        if (asso->associated()) {
            ...
            std::cout << "Measurement z" << measurements[asso->measurement].idx
                      ...
                      << gtsam::Symbol(
                             gt_lmk2map_lmk_.find(meas_lmk_assos_[measurements[asso->measurement].idx])->second)
                      << "\n";
        }
    }
    std::cout << "\n";
